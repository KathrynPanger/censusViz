...
class TractsToFacilities():
    ...
    def get_meters_to_nearest_facility(self):
        ...
        tracts_to_facilities = tracts_cartesian.sjoin_nearest(facilities_cartesian, how="left",
                                                      distance_col="meters_to_nearest_facility")
        # If emissions in data, sort by emissions to get "nearest, then most pollutant plant"
        if self.tracts.varDict["62. ON-SITE RELEASE TOTAL"]:
            tracts_to_facilities = tracts_to_facilities.sort_values(by=[self.tracts.varDict["62. ON-SITE RELEASE TOTAL"]], ascending=False)
        else:
        ...
        if not self.df:
            self.df = tracts_to_facilities
        else:
            self.df = self.tracts_to_facilities.join(tracts_to_facilities, on = "GEOID")
